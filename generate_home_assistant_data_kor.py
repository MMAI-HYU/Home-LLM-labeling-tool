...
FAN_MODES = {
    "english": ["On Low", "On High", "Auto Low", "Auto High", "Off"],
    "korean": ["약풍", "강풍", "자동 약풍", "자동 강풍", "꺼짐"]
    }
...
class ClimateDeviceType(DeviceType):
    def __init__(self, language='korean'):
        ...
        random_parameter_generator={
            "fan_mode": lambda: random.choice(FAN_MODES[language]),
...
SUPPORTED_DEVICES = {
    # --- 기존 기기 수정 ---
    "smartLedBulb": LightDeviceType(), # 이름 변경: light -> smartLedBulb
    "light": LightDeviceType(), # 이름 변경: light -> smartLedBulb
    "lighting": LightDeviceType(),    # lighting 추가 (동일한 LightDeviceType 사용)
    "lightSwitch": DeviceType(        # 이름 변경: switch -> smartSwitch
        name="smartSwitch",
        possible_states=[
            (STATE_ON, 0.5),
            (STATE_OFF, 0.5)
    ...
    "smartSwitch": DeviceType(        # 이름 변경: switch -> smartSwitch
        name="smartSwitch",
        possible_states=[
            (STATE_ON, 0.5),
            (STATE_OFF, 0.5)
    ...
    "smartFanController": DeviceType( # 이름 변경: fan -> smartFanController
    ...
    "smartGarageDoorOpener": DeviceType( # 이름 변경: garage_door -> smartGarageDoorOpener
        name="smartGarageDoorOpener",
        possible_states=[
            (STATE_OPEN, 0.49),
            (STATE_CLOSED, 0.49),
            (STATE_OPENING, 0.01),
    ...
    "smartCurtainTrackKit": DeviceType(
    # 1. 기기 이름 정의
    name="smartCurtainTrackKit",

    # 2. 가능한 상태들 정의 (열림, 닫힘, 열리는 중, 닫히는 중)
    possible_states=[
        (STATE_OPEN, 0.49),
        (STATE_CLOSED, 0.49),
        (STATE_OPENING, 0.01),
    ...
    services={
        ...
        "stop": [],          # 동작 멈춤
        "toggle": [],        # 열림/닫힘 전환
        "set_position": ["position"], # 특정 위치(%)로 이동
    },
    
    # 4. 'set_position'을 위한 임의의 숫자(0~100%) 생성기
    random_parameter_generator={
        "position": lambda: random.randint(0, 100),
    }
),
"kitchenTv": DeviceType(
    # 1. 기기 이름 정의
    name="kitchenTv",

    # 2. 가능한 상태들 정의 (단순하게 켜짐/꺼짐)
    possible_states=[
        (STATE_ON, 0.5),
        (STATE_OFF, 0.5),
    ],
    ...
    services={
        "turn_on": [],
        "turn_off": [],
        "increase_volume": [],          # 볼륨 올리기
        "decrease_volume": [],          # 볼륨 내리기
        "set_volume": ["volume"],       # 특정 값으로 볼륨 설정
        "set_channel": ["channel"],     # 특정 값으로 채널 설정
    },

    # 4. 'set_volume'과 'set_channel'을 위한 임의의 숫자 생성기
    random_parameter_generator={
        "volume": lambda: random.randint(0, 100),   # 0~100 사이의 임의의 볼륨 값
        "channel": lambda: random.randint(1, 200),  # 1~200 사이의 임의의 채널 값
    }
),

    "smartBlindController": DeviceType( # 이름 변경: blinds -> smartBlindController
        name="smartBlindController",
        possible_states=[
            (STATE_OPEN, 0.49),
            (STATE_CLOSED, 0.49),
            (STATE_OPENING, 0.01),
    ...
    "smartDoorlock": DeviceType(     # 이름 변경: lock -> smartDoorlock
        name="smartDoorlock",
        possible_states=[
            (STATE_LOCKED, 0.5),
            (STATE_UNLOCKED, 0.5),
    ...
    "smartThermostat": ClimateDeviceType('korean'), # 이름 변경 및 language 인자 전달
    # --- 우리 데이터셋에 있던 새로운 기기들 추가 ---
    "smartPowerPlug": DeviceType(
        name="smartPowerPlug",
        possible_states=[(STATE_ON, 0.5), (STATE_OFF, 0.5)],
        services={"turn_on": [], "turn_off": [], "toggle": []},
    ),
    "gasShutoffValve": DeviceType(
        name="gasShutoffValve",
        possible_states=[(STATE_OPEN, 0.9), (STATE_CLOSED, 0.1)],
        services={"close": []}, # 가스는 잠그는 동작만 정의했었음
    ),
    "elevatorController": DeviceType(
        name="elevatorController",
        possible_states=[(STATE_IDLE, 1.0)], # 상태는 크게 의미 없으므로 idle로 고정
        services={"call": []},
    ),
"ventilationSystem": DeviceType(
    # 1. 기기 이름 정의
...
"standbyPowerCutoff": DeviceType(
    # 1. 기기 이름 정의
    name="standbyPowerCutoff",
...
BABEL_FORMAT = {
    ...
    "korean": "yyyy년 MMMM d일 EEEE a h시 m분",
...
def generate_static_example(action: dict, persona: str, max_devices: int = 8):
...
def generate_templated_example(template: dict, persona: str, max_devices: int = 8):
...
def generate_status_request(template: dict, persona: str, max_devices: int = 8, return_target_device: bool = False):
...
def generate_dpo_wrong_argument(template: dict, persona: str, max_devices: int = 8):
...
def generate_dpo_no_service_call(template: dict, persona: str, max_devices: int = 8):
...
def generate_dpo_extra_service_call(template: dict, persona: str, max_devices: int = 8):
...
def format_alpaca(example, format_func: Callable):
    ...
    device_list, device_types, extra_exposed_attributes = random_device_list(
        max_devices=8, avoid_device_names=[])
